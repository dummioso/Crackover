...
if git clone "$REPO_URL" crossover_patch; then
    ...
    if clang -dynamiclib -arch arm64 -arch x86_64 -framework Foundation -framework AppKit -o hook.dylib hook.m; then
        ...
        echo "compiler is being annoying (probably that macos 12.1 sdk bug) or you dont have clang,"
        echo "so we're gonna download the pre-built version from github directly"
